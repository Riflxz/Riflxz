...
func handleListSender(ctx context.Context, evt *events.Message) {
	...
	for _, s := range senders {
		...
		call := "nganggur"
		// Fix data race: sess.active & target dilindungi sess.mu (bukan s.mu).
		s.mu.Lock()
		sess := s.sess
		vsess := s.videoSess
		s.mu.Unlock()
		if sess != nil {
			sess.mu.Lock()
			active, target := sess.active, sess.target
			sess.mu.Unlock()
			if active {
				call = "📞 nelpon +" + target
			}
		}
		if call == "nganggur" && vsess != nil && vsess.isActive() {
			call = "🎬 video call"
		}
...
func handlePrank(ctx context.Context, evt *events.Message, args string) {
	...
	sess.pranking = true
	sess.prankFile = mp3Path // Fix RC-FILE-03: simpan path biar OnFinish bisa hapus
	p := sess.player
	orig := sess.nowTitle
	sess.mu.Unlock()
