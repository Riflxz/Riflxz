...
func downloadVideo(downloadURL string) (string, error) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if lastErr == nil {
			// Fix: verifikasi file hasil download gak 0-byte — API theresav
			// kadang balikin 200 dengan body kosong; file kosong diupload
			// nanti cuma bikin error aneh di WhatsApp.
			if fi, err := os.Stat(path); err != nil || fi.Size() == 0 {
				lastErr = fmt.Errorf("file hasil download kosong")
				if err != nil {
					lastErr = err
				}
				os.Remove(path)
		...
		pool.logger.Warn().Int("attempt", attempt).Err(lastErr).Msg("downloadVideo attempt gagal")
		os.Remove(path)
		if attempt < 3 {
