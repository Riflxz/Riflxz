...
// ExecuteNewsletterMutation mengirim query/mutation w:mex arbitrary ke server
// WhatsApp. Ini wrapper publik untuk sendMexIQ — dipakai eksperimen fitur yang
// belum di-expose whatsmeow (mis. update status channel via mutation graphql).
// NOTE: patch lokal vendor — hilang kalau `go mod vendor` dijalankan ulang.
func (cli *Client) ExecuteNewsletterMutation(ctx context.Context, queryID string, variables any) (json.RawMessage, error) {
