...
// probeVideoSize ambil resolusi (width, height) stream video pertama via ffprobe.
// (0,0) kalau gagal / bukan video. Murah (beberapa ms) — dipakai handleHDVid buat
// nentuin mode enhance: video kecil di-upscale, video gede cukup di-sharpen.
...
func extractVideoAssets(mp4Path string) (h264Path, audioPath string, err error) {
	...
	// Encode 600s video di preset veryfast biasanya ≤ beberapa menit; timeout
	// 900s (15 menit) biar proses yang nge-hang (input rusak, codec stuck) tetap
	// bisa mati dan gak nge-gantung .playvideo selamanya.
	...
	if err := cmd.Run(); err != nil {
		...
		if ctx.Err() == context.DeadlineExceeded {
			return "", "", fmt.Errorf("ffmpeg encode timeout (15 menit)")
		}
...
// runCmdTimeout jalanin command dengan timeout — ffmpeg yang makan input rusak
// bisa hang selamanya (blocking di stdin/pipe), jadi semua pemanggilan ffmpeg
// dari handler WA WAJIB lewat sini.
func runCmdTimeout(cmd *exec.Cmd, timeout time.Duration) error {
	...
	case <-time.After(timeout):
		// Kill harus jalan bareng Wait — kalau cuma Kill, proses zombie
		// numpuk. cmd.Wait() di goroutine akan ngambil exit status-nya.
		_ = cmd.Process.Kill()
		<-done
		return fmt.Errorf("timeout setelah %s", timeout)
	}
}
